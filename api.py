...
FULLTEXT_SEARCH_PROMPTS = {
    "query": "Введите поисковый запрос (например: sony ноутбук, кухня):",
}
...
class Bot(telebot.TeleBot):  # TODO добавить status чтобы сразу отправлять есть юзер в базе или нет
    ...
    def complete_answering(self, user_id: int, context: str, data: dict, message=None):
        ...
        if context == 'add':
            ...
            text = HomeManager().search(query=data["query"], user_id=user_id)
    ...
    def _process_step(self, user_id: int, text: str | None = None, step_override=None, value_override=None, message=None):
        ...
        # --- ACTION == SEARCH (full-text via Elasticsearch) ---
        if action == 'search':
    ...
    def register_handlers(self):
        ...
        @self.message_handler(commands=['search'])
        def search(message):
            user = message.from_user
            self.user_states[user.id] = {
                "step": "query",
            ...
            self.reply_to(message, FULLTEXT_SEARCH_PROMPTS["query"])
