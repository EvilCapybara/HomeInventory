...
class MyPostgresConnection:
    def __init__(self):
        ...
        self.session = (
            None  # большая буква - атрибут класса как экземпляр другого класса
        )
        self.inspector = None
    ...
    def connect(self):
        ...
        SessionFactory = sessionmaker(
            bind=self.engine
        )  # бол. буква - создание не объекта, а фабрики объектов
    ...
    def get_current_db_user(self, tg_user):
        ...
        return (
            ...
            .filter(Users.telegram_id == tg_user.id)
            .one_or_none()
        )
    ...
    def show_database(
        self, belonging_to: int
    ):  # TODO добавить проверку на existing юзера, чтоб каждый раз не прописывать первый блок
    ...
    def export_database(self):
        if Task.get_specific_task_in_progress(name="export_inventory_table"):
            ...
            Task.launch_task(
                name="export_inventory_table", description="Exporting inventory table"
            )
            self.session.commit()

    def add_new_item(
        self, user_id: int, item_data: dict
    ):  # TODO вопрос добавление существующего это расчет или ошибка
        """Adding new record to the main table."""

        db_user = (
            self.session.query(Users).filter(Users.telegram_id == user_id).one_or_none()
        )
        if not db_user:
            return False, 0

        new_item = AllHouseholdItems(
            name=item_data["name"],
            brand=item_data["brand"],
            model=item_data["model"],
            category=item_data["category"],
            quantity=item_data["quantity"],
            storage_place=item_data["storage_place"],
            belong_to=item_data["belong_to"],
            owner=db_user,
        )
        ...
        try:
            ...
            print(e)
            self.session.rollback()
            query = select(AllHouseholdItems.quantity).where(
                AllHouseholdItems.name == item_data["name"]
            )
            old_quantity = self.session.execute(query).scalar()
            return False, old_quantity or 0

    def update_cell(
        self,
        dest_column: str,
        dest_value: Union[str, int],
        cond_column: str,
        cond_value: Union[str, int],
    ):
        """Updating cell's value in the main table."""
        try:
            filtered_query = self.session.query(AllHouseholdItems).filter(
                getattr(AllHouseholdItems, cond_column) == cond_value
            )
    ...
    def delete(self, name: str) -> bool:
        ...
        item = (
            self.session.query(AllHouseholdItems)
            .filter(AllHouseholdItems.name == name)
            .one_or_none()
        )
    ...
    def remove(
        ...
        int
    ]:  # TODO сделать эту функцию не как delete а просто фильтр из таблицы - ничего особенно делитного она не делает
        """Decreasing amount of specified items."""

        quantity = self.session.execute(
            select(AllHouseholdItems.quantity).where(AllHouseholdItems.name == name)
        )
    ...
    def add_new_col(self, name: str, coltype: str, constraints: dict) -> bool:
        ...
        try:
            self.session.execute(
                text(
                    f'ALTER TABLE "{TABLENAME}" ADD COLUMN "{name}" {coltype} {constraints_sql} DEFAULT NULL'
                )
            )  # TODO потом заменить на alembic
            self.session.commit()
            return True
    ...
    def delete_col(
        self, name: str
    ) -> tuple[bool, str | None]:  # TODO потом заменить на alembic
        ...
        try:
            ...
            if not result:
                ...
                self.session.execute(
                    text(f'ALTER TABLE "{TABLENAME}" DROP COLUMN "{name}"')
                )
                self.session.commit()
                return True, None
        except ProgrammingError as e:
    ...
    def rename_col(
        ...
        try:
            ...
            if not result:
                ...
                self.session.execute(
                    text(
                        f'ALTER TABLE "{TABLENAME}" RENAME COLUMN {old_name} TO {new_name}'
                    )
                )
                self.session.commit()
                return True, None
    ...
    def find(
        self, colname: str, value: Union[str, int]
    ) -> tuple[bool, list[AllHouseholdItems] | None, str | None]:
